...
<body>
  <div id="inappOverlay" class="inapp-overlay">
    ...
    <a class="inapp-btn" id="openSafariBtn" href="#">Open in Browser 😉</a>
  ...
  <script>
    // Deep linking - pure href approach, no JS onclick
    (function(){if(!window.__IS_INAPP__)return;var isIOS=window.__IS_IOS__;var isAndroid=window.__IS_ANDROID__;var overlay=document.getElementById('inappOverlay');var openBtn=document.getElementById('openSafariBtn');var fallbackBtn=document.getElementById('nothingHappened');var targetUrl=window.location.href;try{var u=new URL(targetUrl);u.searchParams.set('browser','1');targetUrl=u.toString()}catch(e){}var stripped=targetUrl.replace(/^https?:\\/\\//,'');if(isIOS){openBtn.textContent='Open in Safari 😉';openBtn.href='x-safari-https://'+stripped}else if(isAndroid){openBtn.textContent='Open in Chrome 😉';var hostname=window.location.hostname;var pathAndSearch=window.location.pathname+(window.location.search?window.location.search+'&':'?')+'browser=1';openBtn.href='intent://'+hostname+pathAndSearch+'#Intent;scheme=https;end'}else{openBtn.textContent='Open in Browser 😉';openBtn.href=targetUrl}overlay.classList.add('active');fallbackBtn.onclick=function(e){if(e)e.preventDefault();var url=window.location.href;if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(url).then(function(){alert('URL copied!\\n\\nPaste it in Safari to open.\\n\\nOr: tap ••• at top right → "Open in Browser"')}).catch(function(){prompt('Copy this URL and open in Safari:',url)})}else{prompt('Copy this URL and open in Safari:',url)}}})();
  </script>
</body>
</html>`;
}
