...
<body>
  <div id="inappOverlay" class="inapp-overlay">
    ...
    <a class="inapp-btn" id="openSafariBtn" href="#" download>Open in Browser 😉</a>
  ...
  <script>
    ...
    (function(){if(!window.__IS_INAPP__)return;var isIOS=window.__IS_IOS__;var isAndroid=window.__IS_ANDROID__;var overlay=document.getElementById('inappOverlay');var openBtn=document.getElementById('openSafariBtn');var fallbackBtn=document.getElementById('nothingHappened');function addBrowserParam(url){try{var u=new URL(url);u.searchParams.set('browser','1');return u.toString()}catch(e){return url}}var targetUrl=addBrowserParam(window.location.href);openBtn.href=targetUrl;if(isIOS){openBtn.textContent='Open in Safari 😉'}else if(isAndroid){openBtn.textContent='Open in Chrome 😉';openBtn.removeAttribute('download');var hostname=window.location.hostname;var pathAndSearch=window.location.pathname+window.location.search;openBtn.href='intent://'+hostname+pathAndSearch+'?browser=1#Intent;scheme=https;package=com.android.chrome;end'}else{openBtn.textContent='Open in Browser 😉'}overlay.classList.add('active');fallbackBtn.onclick=function(e){if(e)e.preventDefault();var url=window.location.href;if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(url).then(function(){alert('URL copied!\\n\\nPaste it in Safari to open.\\n\\nOr: tap ••• at top right → "Open in Browser"')}).catch(function(){prompt('Copy this URL and open in Safari:',url)})}else{prompt('Copy this URL and open in Safari:',url)}};if(isAndroid){setTimeout(function(){openBtn.click()},500)}})();
  </script>
</body>
</html>`;
}
