...
<body>
  ...
  <script>
    // Deep linking - all known iOS escape methods from inappdebugger.com
    (function(){if(!window.__IS_INAPP__)return;var isIOS=window.__IS_IOS__;var isAndroid=window.__IS_ANDROID__;var overlay=document.getElementById('inappOverlay');var openBtn=document.getElementById('openSafariBtn');var fallbackBtn=document.getElementById('nothingHappened');if(isIOS){openBtn.textContent='Open in Safari 😉'}else if(isAndroid){openBtn.textContent='Open in Chrome 😉'}else{openBtn.textContent='Open in Browser 😉'}overlay.classList.add('active');function addBrowserParam(url){try{var u=new URL(url);u.searchParams.set('browser','1');return u.toString()}catch(e){return url}}function uuid(){return'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,function(c){var r=Math.random()*16|0;return(c==='x'?r:(r&0x3|0x8)).toString(16)})}function handleiOSClick(){var targetUrl=addBrowserParam(window.location.href);var stripped=targetUrl.replace(/^https?:\\/\\//,'');var schemes=['x-safari-https://'+stripped,'googlechromes://'+stripped,'googlechrome://'+stripped,'shortcuts://x-callback-url/run-shortcut?name='+uuid()+'&x-error='+encodeURIComponent(targetUrl)];var attempt=0;var timer=null;function cancelTimer(){if(timer){clearTimeout(timer);timer=null}}document.addEventListener('visibilitychange',function handler(){if(document.visibilityState==='hidden'){document.removeEventListener('visibilitychange',handler);cancelTimer()}});function tryNext(){if(attempt<schemes.length){if(attempt===0){window.location.href=schemes[attempt]}else{window.open(schemes[attempt],'_blank')}attempt++;timer=setTimeout(tryNext,800)}else{window.location.href=targetUrl}}tryNext()}function handleAndroidClick(){var targetUrl=addBrowserParam(window.location.href);var hostname=window.location.hostname;var pathAndSearch=window.location.pathname+window.location.search;var intentUrl='intent://'+hostname+pathAndSearch+'?browser=1#Intent;scheme=https;end';window.location=intentUrl}openBtn.onclick=function(e){if(e)e.preventDefault();if(isIOS)handleiOSClick();else if(isAndroid)handleAndroidClick();else window.open(window.location.href,'_blank')};fallbackBtn.onclick=function(e){if(e)e.preventDefault();var url=window.location.href;if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(url).then(function(){alert('URL copied!\\n\\nPaste it in Safari to open.\\n\\nOr: tap ••• at top right → "Open in Browser"')}).catch(function(){prompt('Copy this URL and open in Safari:',url)})}else{prompt('Copy this URL and open in Safari:',url)}};if(isAndroid){setTimeout(function(){handleAndroidClick()},500)}})();
  </script>
</body>
</html>`;
}
